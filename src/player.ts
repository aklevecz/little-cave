...
class PlayerControls {
  ...
  _keyboardInstructions() {
    ...
    const ps = [
      "Hello! looks like you are on a laptop!",
      "Use the AWSD keys to move",
    ];
    ps.forEach((p) => {
  ...
  _mobileInstructions() {
    ...
    overlay.className = "overlay";
    const ps = [
      "Hello! looks like you are on a phone!",
      "Move the phone to look around and press the button with the running man to move forward",
    ];
    ps.forEach((p) => {
      const pel = document.createElement("p");
      pel.innerText = p;
  ...
  moveRight(distance: number) {
    if (this.isDevice) {
      return ((this.controls as DeviceOrientationControls).object.position.x +=
        distance);
    } else {
...
export class Player {
  ...
  _mobileMoveControls() {
    ...
    this._controls.mobileMoveButton.addEventListener("touchend", onTouchEnd);
  }
  ...
  Update(delta: number) {
    ...
    if (this._controls.isLocked) {
      ...
      if (intersections.length > 0) {
        ...
        if (teemo.length > 0) {
          if (
            teemo[0].position.distanceTo(this._controls.getObject().position) <
            height + 9
          ) {
            return;
          }
